...
class BitArray {
    constructor(size) {
        if (!Number.isInteger(size))
            throw new TypeError('new BitArray(size): size must be an integer');
        // -- end error checking --
        this.size = size;
        this.map = new Map();
    }
    ...
    set(i, val) {
        if ((!Number.isInteger(i)) || (!Number.isInteger(val)))
            throw new TypeError('i and val must be an integers');
        if (i >= this.size || i < 0) {
